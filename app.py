...
with col_up2:
    arq_lista = st.file_uploader("Seu LISTÃO DE PREÇOS", type=["xlsx", "csv"])
...
if arq_obra and arq_lista:
    try:
        # Lendo a Obra (pula 7 linhas) - Forçamos o header=0 após o skip para pegar a linha 8
        if arq_obra.name.endswith('.csv'):
            df_obra = pd.read_csv(arq_obra, skiprows=7)
        else:
            # header=0 garante que a linha 8 (índice 7) seja o título das colunas
            df_obra = pd.read_excel(arq_obra, skiprows=7)
        
        # Limpeza de colunas sem nome (Unnamed)
        ...
        st.success(f"✅ Planilha carregada: {len(df_obra)} linhas identificadas.")

        # --- 5. BUSCADOR ---
        with st.expander("🔍 CONSULTAR PREÇOS NO LISTÃO"):
            termo = st.text_input("Pesquise no seu Listão:")
            if termo:
        ...
        # --- 7. CÁLCULOS ---
        # Tenta localizar a coluna de quantidade dinamicamente
    ...
    st.warning("Aguardando os ficheiros...")
