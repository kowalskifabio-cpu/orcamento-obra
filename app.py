...
# --- 2. FRAGMENTO PARA MANTER A TELA ABERTA ---
@st.fragment
def renderizar_conteudo_cpu(idx, linha_master):
    ...
    def processar_bloco(titulo, chave, tipo_fator):
        ...
        # Processamento de cálculos e busca
        if not df_ed.equals(df):
            ...
            st.rerun(scope="fragment") # Atualiza apenas o fragmento (caixa aberta)
    ...
    if st.button("Salvar Tudo e Sair da Caixa"):
        st.session_state.df_obra.at[idx, 'CUSTO UNITÁRIO FINAL'] = total_item
        st.session_state.df_obra.at[idx, 'STATUS'] = "✅"
        st.rerun(scope="app") # Fecha a caixa e atualiza a planilha master

# --- 3. MODAL DE COMPOSIÇÃO ---
@st.dialog("Composição Técnica", width="large")
def abrir_cpu(idx, linha_master):
    renderizar_conteudo_cpu(idx, linha_master)

# --- 4. INTERFACE PRINCIPAL ---
st.title("Orçamentador Marcenaria v4")
c1, c2 = st.columns(2)
